...
#    psql -U postgres --username=${DB_USER} --dbname=${DB_NAME} --host=localhost -c "SELECT create_hypertable('${DB_SCHEMA}.${DB_TABLE}',by_range('${DB_TIMESTAMP_COLUMN}', INTERVAL '${DB_INTERVAL}'));"
#    psql -U postgres --username=${DB_USER} --dbname=${DB_NAME} --host=localhost -c "SELECT add_retention_policy('${DB_SCHEMA}.${DB_TABLE}', drop_after => INTERVAL '${DB_DROP_INTERVAL}');"
